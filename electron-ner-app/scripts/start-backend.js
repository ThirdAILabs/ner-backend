...
function getBinPath() {
  ...
  for (const primaryLocation of primaryLocations) {
    ...
    if (fs.existsSync(primaryLocation)) {
      log.debug(':white_check_mark: Found bin at primary location');
  ...
  for (const possiblePath of fallbackPaths) {
    ...
    if (fs.existsSync(possiblePath)) {
      log.debug(':white_check_mark: Found bin at fallback path:', possiblePath);
      return possiblePath;
    }
  }
  ...
  for (const stdPath of standardMacPaths) {
    ...
    if (fs.existsSync(stdPath)) {
      log.debug(':white_check_mark: Found bin at standard path:', stdPath);
      return stdPath;
    }
  }
  ...
  log.error(':x: Could not find bin directory in any location');
  return null;
}
...
export async function startBackend() {
  ...
  if (!portAvailable) {
    log.error(`:red_circle: Could not secure port ${FIXED_PORT} for backend use`);
    return null;
  }

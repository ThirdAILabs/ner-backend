...
export const PerformanceSummary: React.FC<PerformanceSummaryProps> = ({
  ...
  const calculateChange = (before: number, after: number) => {
    return {
      value: after - before,
      percentage:
        before === 0 ? 100 : Math.round(((after - before) / before) * 100)
    };
  };

  const precisionChange = calculateChange(
    beforeMetrics.precision,
    afterMetrics.precision
  );
  const recallChange = calculateChange(
  ...
  return (
    <Box sx={{ mb: 4 }}>
      ...
      <Paper
        sx={{ border: '1px solid rgba(0, 0, 0, 0.12)', boxShadow: 'none' }}
      >
        <Grid container>
        ...
        <Box sx={{ p: 2, backgroundColor: 'rgba(0, 0, 0, 0.02)' }}>
          ...
          <Typography variant="body1" sx={{ fontWeight: 500, mt: 0.5 }}>
            ...
            {f1Change.value > 0 ? 'improved by' : 'decreased by'}{' '}

...
const calculateProgress = (report: any | null): number => {
...
const getProcessedTokens = (report: any | null): number => {
...
export default function JobDetailClient({ deploymentId, jobId }: { deploymentId: string, jobId: string }) {
  ...
  const fetchTags = async () => {
    ...
    try {
      ...
      const report = await nerService.getReport(jobId);
      ...
      if (report.CustomTags !== undefined) {
        ...
        const allCustomTags: any[] = customTagName.map((tag) => ({
          name: tag,
  ...
  const loadRealClassifiedTokenRecords = async (offset = 0, limit = 50) => {
    try {
      const entities = await nerService.getReportEntities(jobId, { offset, limit });
      ...
      const uniqueTypes = new Set(entities.map((entity: any) => entity.Label));
      ...
      return entities.map((entity: any) => {
        const record = {
          ...
          groups: reportData?.Groups?.filter((group: any) => group.Objects?.includes(entity.Object)).map((g: any) => g.Name) || []
        };
  ...
  const loadRealObjectRecords = async () => {
    try {
      ...
      const objectPreviews = await nerService.getReportObjects(jobId, { limit: 100 });

      return objectPreviews.map((preview: any) => {
        // Create tagged tokens array from the parallel tokens and tags arrays
        const taggedTokens = preview.tokens.map((token: string, index: number) => [
          token,
          preview.tags[index]
        ]) as [string, string][];
        ...
        return {
          ...
          groups: reportData?.Groups?.filter((group: any) =>
      ...
      <Tabs value={tabValue} onValueChange={setTabValue} className="w-full">
        ...
        <TabsContent value="output">
          <DatabaseTable
            ...
            groups={reportData?.Groups?.map((g: any) => g.Name) || mockGroups}
            tags={availableTags}
          />
        </TabsContent>
      </Tabs>

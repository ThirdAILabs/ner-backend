...
func TestListModels(t *testing.T) {
	...
	service := backend.NewBackendService(db, &mockStorage{}, "uploads", messaging.NewInMemoryQueue(), 1024, nil, storage.ConnectorConfigs{})
...
func TestGetInferenceMetrics_NoTasks(t *testing.T) {
	...
	svc := backend.NewBackendService(db, &mockStorage{}, "uploads", nil /*chunkTargetBytes*/, 0, nil, storage.ConnectorConfigs{})
	router := chi.NewRouter()
...
func TestGetInferenceMetrics_WithTasks(t *testing.T) {
	...
	svc := backend.NewBackendService(db, &mockStorage{}, "uploads", nil, 0, nil, storage.ConnectorConfigs{})
	router := chi.NewRouter()
	svc.AddRoutes(router)
...
func TestValidateGroupDefinition_ValidDefinition(t *testing.T) {
	service := backend.NewBackendService(nil, &mockStorage{}, "uploads", messaging.NewInMemoryQueue(), 1024, nil, storage.ConnectorConfigs{})
	router := chi.NewRouter()
	service.AddRoutes(router)
